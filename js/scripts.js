    ...
    for (var index = 0; index < listaNotas.length; index++) {
        ...
        if (listaNotas[index].editando == true) {
            ...
            inserirHTML += '<form class="note note--editing">' +
                '<input class="note__title note__title--editing" type="text" name="title" placeholder="Título" value="' + listaNotas[index].titulo + '" autofocus />' +
                '<textarea class="note__body note__body--editing" name="body" rows="5" placeholder="Criar uma nota...">' + listaNotas[index].texto + '</textarea>' +
                '<button class="note__control" type="button" onclick="adicionarNota(this.form.title, this.form.body, this.form, this.form.parentElement,' + index + ')"> Salvar </button>' +
                '</form>'
...
function adicionarNota(inputTitulo, inputTexto, formulario, secao, index) {
    ...
    if (listaNotas[index]) {         // não precisa colocar o == true 
